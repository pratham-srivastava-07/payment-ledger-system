...
export class DatabaseInboxTransport implements EventTransport {
  ...
  async publish(event: OutboxEvent) {
    ...
    await this.db.inboxEvent.createMany({
      data: [{
        id: event.id, eventType: event.eventType, version: event.version,
...
export class OutboxPublisher {
  ...
  constructor(private db: PrismaClient = prisma, transport?: EventTransport, private maxAttempts = 8) {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const event = await this.db.$transaction(async (tx) => {
        ...
        if (rows[0].attempts >= this.maxAttempts) {
          await tx.outboxEvent.update({ where: { id: rows[0].id }, data: {
        ...
        return tx.outboxEvent.update({ where: { id: rows[0].id }, data: {
          attempts: { increment: 1 }, leaseToken: token, leaseUntil: new Date(Date.now() + 30000),
        } });
      });
      if (!event) break;
      try {
        await this.transport.publish(event);
        await this.db.outboxEvent.updateMany({ where: { id: event.id, leaseToken: token }, data: {
          status: "PUBLISHED", publishedAt: new Date(), leaseToken: null, leaseUntil: null, lastError: null,
        } });
        count++;
      } catch (error) {
        const delay = Math.min(60000, 1000 * 2 ** Math.min(event.attempts - 1, 6)) + Math.floor(Math.random() * 250);
        await this.db.outboxEvent.updateMany({ where: { id: event.id, leaseToken: token }, data: {
          status: event.attempts >= this.maxAttempts ? "FAILED" : "PENDING",
          availableAt: new Date(Date.now() + delay), leaseToken: null, leaseUntil: null,
  ...
  async retryFailed(id: string) {
    return this.db.outboxEvent.updateMany({ where: { id, status: "FAILED" }, data: {
      status: "PENDING", attempts: 0, availableAt: new Date(), leaseToken: null, leaseUntil: null,
    } });
  }
}
