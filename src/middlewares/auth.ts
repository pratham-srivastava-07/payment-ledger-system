...
export class AuthMiddleware {
  static verifyToken(req: Request, res: Response, next: NextFunction) {
    ...
    if (!secret) throw new DomainError("AUTH_NOT_CONFIGURED", "Authentication is not configured", 503);
    ...
    try {
      ...
      if (typeof decoded === "string" || !decoded.sub || decoded.sub.length > 128) throw new Error("Missing subject");
